...
const {findUserById, findUserByEmail, saveNewuserDetails} = require('../controllers/auth.controller.js')
...
.post(async(req, res)=> {
    try {
        ...
        const user = await findUserByEmail(email);
        if(!user){
            res.status(404).json({success: false, message: "email/user does not exists"})
        }
